...
def selectzyBook(driver):
    while (True):
        ...
        try:
            ...
            zybook_selection = driver.find_element_by_xpath("//a[@href='/zybook/" + course + "?selectedPanel=student-activity']")
...
skip_completed = True
browser = "Firefox" #todo: give options for different installed browsers
...
try:
    ...
    try:
        print('attempting to load zybook')
        WebDriverWait(driver, 10).until(expected_conditions.element_to_be_clickable((By.CSS_SELECTOR, ".zybook")))
        print('loaded zybook')
    except:
